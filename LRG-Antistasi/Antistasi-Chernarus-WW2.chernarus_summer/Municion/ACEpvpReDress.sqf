...
} forEach [["ACE_CableTie",1],["ACE_MapTools",1]];
...
if (hayACEMedical) then
	...
	if !([player] call A3A_fnc_isMedic) then
		...
		} forEach [["ACE_morphine",15],["ACE_epinephrine",9],["ACE_salineIV",8],["ACE_fieldDressing",30]];
		if (ace_medical_level == 2) then
			{
			{
			_item = _x select 0;
			...
			} forEach [["ACE_elasticBandage",20],["ACE_packingBandage",10],["ACE_epinephrine",5],["ACE_morphine",5],["ACE_tourniquet",10],["ACE_salineIV_250",2],["ACE_surgicalKit",1],["ACE_personalAidKit",2]];
			};
		};
	};
