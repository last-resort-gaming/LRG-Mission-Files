...
G_Revive_AI_Incapacitated = [WEST]; //Array of sides of AI that will utilize revive system [WEST, EAST, RESISTANCE, CIVILIAN]
...
G_Revive_DownsPerLife = 4; //Number of times unit can go Incapacitated in single life. 0 = Unlimited, integer > 0 = limit of downs per life.
...
G_Revive_actionTime = 15; //Time (in seconds) required for reviver to complete revive action
...
G_Squad_Leader_Marker = true; //Displays marker on map indicating squad leader's position. true = enabled, false = disabled.
	...
	G_Squad_Leader_Mkr_Color = "ColorBlue"; //Color of marker
...
G_AI_Fixed_Spawn = false; //Upon respawn, the AI will spawn at the marker defined below for their respective side, as opposed to respawning at random if multiple markers exist
...
G_Unit_Tag = true; //Refers to unit "name tags" that display over unit's head on HUD. Only friendlies visible. true = enabled, false = disabled.
	G_Unit_Tag_Display = 1; //0 = Press defined key to have names visible for defined time, 1 = Cursor over unit to have name displayed, 2 = Names always displayed
	...
	G_Unit_Tag_Distance = 30; //Distance from player that marker will begin to appear
	G_Unit_Tag_ShowDistance = false; //Distance is displayed next to player's name
	G_Unit_Tag_Color = [1,1,0.1]; //RGB settings for the tag color of non-squad members. Alpha is normally the 4th number, but that is handled in the script via a distance formula.
	G_Unit_Tag_SquadColor = [1,1,1]; //RGB settings for the tag color of squad members. Alpha is normally the 4th number, but that is handled in the script via a distance formula.
