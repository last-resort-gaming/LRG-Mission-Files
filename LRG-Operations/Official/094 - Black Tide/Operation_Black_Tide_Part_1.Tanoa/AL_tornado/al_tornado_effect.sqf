...
enableCamShake true;
[_tsource] spawn {_torsouce = _this select 0;while {!isNull _torsouce} do {if ((player distance _torsouce) < 500) then {addCamShake [0.3,7,11];sleep 10 + random 60;};};};
// vartej alb
...
While {tornadosino != "goof"} do {effect_screen = ppEffectCreate ["FilmGrain", 2000];effect_screen ppEffectEnable true;effect_screen ppEffectAdjust [0.1,0.1,0.5,0.1,0.1,true];effect_screen ppEffectCommit 0;sleep 0.5};	
