...
[[_tornadasource,_tornadamid],"AL_tornado\al_tornado_effect.sqf"] remoteExec ["execVM",0,true];
[[_tornadamid],"AL_tornado\tornadoSound.sqf"] remoteExec ["execVM",0,true];
[_tornadasource] spawn {_tuner_tor = _this select 0;while {!isNull _tuner_tor} do {	[[_tuner_tor],"AL_tornado\al_tunet_tornado.sqf"] remoteExec ["execVM",0,true];sleep 3}};
...
while {tornadosino == "goon"} do
{
	...
	_fct = [90,-90] call BIS_fnc_selectRandom;
	...
	{if(_x != _tornadasource) then {if((_x isKindOf "LandVehicle") or (_x isKindOf "Man") or (_x isKindOf "Air")) then {_f=[-1,1]call BIS_fnc_selectRandom; _x setvelocity [(random 10) * _f,(random 10)* _f,20+(random 10)* _f]; _x setdamage 0.5; sleep 0.5;} else {_x setdamage [1,false]}}} foreach _nearobjects;	
	if (_tornadasource distance (getMarkerPos _tornado_dest) < 20) then	{tornadosino = "goof"; publicvariable "tornadosino"}; 
	sleep 1;
	_tornadamid setPosATL [getPosATL _tornadasource select 0,getPosATL _tornadasource select 1,100];
	sleep 0.5;
};
	[_tornadasource,["uragan_end",2000]] remoteExec ["say3d"];
