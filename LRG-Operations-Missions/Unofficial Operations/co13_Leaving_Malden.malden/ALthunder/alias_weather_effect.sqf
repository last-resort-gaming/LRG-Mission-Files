...
while {true} do {
	...
	_alias_rain_drop_factor=linearConversion [0.1,1,rain,0.04,0.0035,true]; //hint str _alias_rain_drop_factor;
...
if (alias_wind_debris) then 
{
	[] spawn 
	{
		...
		//waitUntil {!(missionNamespace getVariable "in_house")};
		//_alias_debris_interval = 180 + random 300;
		_alias_debris_interval = 10 + random 10;
		_alias_wind = wind;
		...
		_leaves_p setParticleParams [["\A3\data_f\ParticleEffects\Hit_Leaves\Sticks_Green", 1, 1, 1], "", "SpaceObject", 1,17,[0,-5,15],[_xx_speed,_yy_speed,0],0,0.00001,0.0,0.1,[random 5],[[0.68,0.68,0.68,1],[0.68,0.68,0.68,0]],[1.5,1],13,13,"","",vehicle player,0,true,1,[[0,0,0,0]]];
		*/
