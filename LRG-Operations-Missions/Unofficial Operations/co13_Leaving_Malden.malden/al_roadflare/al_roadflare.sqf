...
// [[[_al_throwable],"AL_roadflare\al_roadflare.sqf"],"BIS_fnc_execVM",true,true] spawn BIS_fnc_MP;
...
switch (typeOf _al_throwable) do {
    case "Chemlight_blue": {if (al_chem) then {/*hint "Blue Chemlight";*/_al_color_flare = [0.2,0.6,1]};};
...
if (!_al_roadflare) then {
	...
	if (al_chem) then {
		...
		_al_throwable_light setLightIntensity 10;
		_al_throwable_light setLightBrightness 3;
		_al_throwable_light setLightUseFlare false;
		_al_throwable_light setLightFlareSize 0.5;
		_al_throwable_light setLightFlareMaxDistance 600;
