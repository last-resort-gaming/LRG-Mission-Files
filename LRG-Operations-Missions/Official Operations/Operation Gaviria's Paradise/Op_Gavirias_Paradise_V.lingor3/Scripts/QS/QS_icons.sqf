...
if (_PlayerGroup == "Command") then {_QS_ST_showGroupOnly = False;};
...
_QS_ST_iconUpdatePulseDelay = 180;								// NUMBER. How often should location of unit on the MAP be updated? 0 = as fast as possible, else if > 0 then it = time in seconds. Default 0.
...
_QS_ST_GPSDist = 10;											// NUMBER. Distance from player that units shown on GPS. Higher number = lower script performance. Not significant but every 1/10th of a frame counts! Default 300
