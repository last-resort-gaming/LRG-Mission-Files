...
if (YAINA_PlugsIn isEqualTo true) then {
    ...
    if (!isNil "_target") then {
        [" <img image='f\earplugs\plugs_in.paa' /><br/><t valign='middle' align='center' size='.4'>Earplugs Inserted</t>",0,0.6, 2,1,0,0] spawn BIS_fnc_dynamicText;
    };
} else {
    ([2, 0] select isNil "_target") fadeSound 1;
    if (!isNil "_target") then {
        ["<img image='f\earplugs\plugs_out.paa' /><br/><t valign='middle' align='center' size='.4'>Earplugs Removed</t>",0,0.6, 2,1,0,0] spawn BIS_fnc_dynamicText;
    };
};
...
_aid = player addAction [("<t color=""" + _nextColor + """>" + _nextAction + " Earplugs</t>"),"f\earplugs\earplugs.sqf","",-98,false,true,"",'_target isEqualTo vehicle _this'];
