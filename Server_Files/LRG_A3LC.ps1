...
Start-Process -FilePath "$Server_Dir\arma3server_x64.exe" -ArgumentList "-profiles=`"$Server_Profiles`" -port=$Server_port -name=LRGServer `"-config=$Server_Config`" -autoinit -limitFPS=200 `"-ServerMod=$ServerMods`" `"-Mod=$ClientMods`""
