...
*/
	...
	if !(EnableSideChat) exitwith {};
	if !(isClass (configFile >> "CfgPatches" >> "task_force_radio")) exitwith {};
		
	PlayerGroup =  group player getVariable "LRG_section";


if (PlayerGroup in SideChatGroups) then { 1 enableChannel [true, false];};
	
_handle = [
{
	
		if (servertime > (SideChatDuration * 60)) exitwith {
